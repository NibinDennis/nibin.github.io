...
(function () {
    ...
    app.controller('CatalogueController', function ($scope, $http, $timeout, $window, $filter, $location,$rootScope) {
        ...
        $scope.checkshort = function (index) {
            ...
            if ($('#chks' + index).is(':checked') == true ) {
                $scope.rows[index].s = 1;

               //alert($('#chks' + index).is(':checked'));

                  }
            if ($('#chks' + index).is(':checked')==false) {
                   ...
                   params :{Noun:$scope.cat.Noun,Modifier: $scope.cat.Modifier}
                    ...
                    if (response != '') {
                        ...
                        angular.forEach($scope.Characteristics, function (value1) {
                            angular.forEach($scope.newtempchar, function (ch) {
                                if (value1.Characteristic === ch.Characteristic) {
                                    value1.Value = ch.Value;
                                }
                            });
                        });
                        angular.forEach($scope.Characteristics, function (value1) {
                            angular.forEach($scope.tempchar, function (ch) {
                                if (value1.Characteristic === ch.Characteristic) {
                                    value1.Value = ch.Value;
                                }
                            });
                        ...
                        angular.forEach($scope.Characteristics, function (value1) {
                            $scope.newchars.push(value1.Characteristic)
                        });
       ...
       // $scope.GetUserList();
        ...
        $scope.loadmodifier = function (noun) {
            ...
            $scope.Characteristics = null;
            $scope.cat.RevRemarks = null; $scope.cat.RelRemarks = null; $scope.cat.EnrichedValue = null;
        ...
        $scope.RowClick = function (lst, idx) {
            ...
            //$rootScope.cgBusyPromises =   $http({
            //    method: 'Post',
            //    url: '/Catalogue/SaveImg',
            //    params: { EquipId: lst.Itemcode }
            //}).success(function (response) {
            //   // $scope.asset = response;
            //    angular.forEach(response, function (lst1) {
            //        $scope.asset.push(lst1.Appimage)
            //    });
            //  //  $scope.getImg(lst.Itemcode);
            //}).error(function (data, status, headers, config) {
            //    // alert("error");
            //});
            ...
            $http({
               ...
               // $('#chks' + i).attr("style", "");
                if ($scope.cat.Unspsc != null || $scope.cat.Unspsc != "") {
                    $http({
                        ...
                        if (response != '') {
                            ...
                            if ($scope.Commodities[0].Commodity != null && response[0].Commodity != "") {
                                ...
                                if ($scope.cat.Unspsc == null || $scope.cat.Unspsc == "" || $scope.cat.Unspsc == undefined)
                                    $scope.cat.Unspsc = $scope.Commodities[0].Commodity;
                                ...
                                if ($scope.cat.Unspsc == null || $scope.cat.Unspsc == "" || $scope.cat.Unspsc == undefined)
                                $scope.cat.Unspsc = $scope.Commodities[0].Class;
                            }
                        }
                        else {
                            $scope.Commodities = null;
                    ...
                    $scope.cat.Unspsc = response.Unspsc;
                }
                        ...
                        angular.forEach($scope.Characteristics, function (value1, key1) {
                            ...
                            angular.forEach($scope.cat.Characteristics, function (value2, key2) {
                                ...
                                if (value1.Characteristic === value2.Characteristic) {
                                    ...
                                    $scope.newtempchar.push({ Characteristic: value2.Characteristic, Value: value2.Value });
                                }
                            });
                        });
        ...
        $scope.checkDuplicate = function (form) {
         //   alert($scope.cat.Catalogue.Name)
           // alert("hai");
        ...
        $scope.checkValue = function (Noun, Modifier, Attribute, Value, indx) {
          //  Value = Value.replace('&', '***');
            $scope.tempchar.push({ Characteristic: Attribute, Value: Value });
        ...
        //$scope.RemoveImg = function (fle, Uid) {
        //   // alert(Uid)
        ...
        //            method: 'GET',
        //            url: '/Catalogue/DeleteImg?FileName=' + fle + "&Uid=" + Uid
